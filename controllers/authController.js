...
const { Auth, Blacklist, Notification } = require("../models/authsModel");
const {
  FreelancerCategory,
...
const registerUser = async (req, res) => {
  try {
    ...
    const usernameSuggestions = await generateUniqueUsernames(
      first_name,
      last_name
    );
...
const sendOtp = async (req, res) => {
  try {
    ...
    if (type !== "mobile" && type !== "email") {
      return res
        .status(400)
        .json({ result: "false", message: "Type must be 'mobile' or 'email'" });
    }
    ...
    if (type === "mobile") {
      if (!value.trim()) {
        return res
          .status(400)
          .json({ result: "false", message: "Mobile number cannot be empty" });
      }
      user.mobile = value;
    } else if (type === "email") {
      ...
      if (!value.trim()) {
        return res
          .status(400)
          .json({ result: "false", message: "Email cannot be empty" });
      }
      user.email = value;
    }
  ...
  try {
    ...
    if (!userId || !type || !otp) {
      return res
        ...
        .json({
          result: "false",
          message: "userId, type, and otp are required",
        });
    }
    ...
    if (user.otp !== otp || new Date() > user.otpExpires) {
      return res
        .status(400)
        .json({ result: "false", message: "Invalid or expired OTP" });
    }
...
const googleLoginWithAccessToken = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      usernameSuggestions = await generateUniqueUsernames(
        first_name,
        last_name
      );
...
const sendOtpforgetPassword = async (req, res) => {
  try {
    ...
    if (!type || !value || (type !== "email" && type !== "mobile")) {
      return res
        ...
        .json({
          result: "false",
          message:
            "Valid 'type' and 'value' are required (type must be 'email' or 'mobile')",
        });
    }
    ...
    const user = await Auth.findOne(
      type === "email" ? { email: input } : { mobile: input }
    );
...
const forgetPassword = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ result: "true", msg: "Password updated successfully" });
  } catch (error) {
...
const freelancercloseAccount = async (req, res) => {
  try {
    ...
    if (!userID) {
      return res
        .status(400)
        .json({ result: false, msg: "Parameter required: userID" });
    }

    if (!mongoose.isValidObjectId(userID)) {
      return res
        .status(400)
        .json({ result: false, msg: "Invalid userID format" });
    }
...
const getUserNotifications = async (req, res) => {
  try {
    ...
    if (!userId) {
      return data
        .status(400)
        .json({ result: "false", message: "No Notification List" });
    }
...
const getAllTopFreelancers = async (req, res) => {
  try {
    const topFreelancers = await Auth.find({ topFreelancer: true }).sort({ createdAt: -1 });

    res.status(200).json({
      result: true,
      msg: "Top freelancers fetched successfully",
      data: topFreelancers,
    });
  } catch (error) {
    res.status(500).json({
      result: false,
      msg: "Failed to fetch top freelancers",
      error: error.message,
    });
  }
};
